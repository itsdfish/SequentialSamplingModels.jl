...
"""
mutable struct MDFT{T<:Real,T1} <: Simulator
    ν::Vector{T}
    S::Array{T,2}
    α::T
    τ::T
...
increment!(dist::MDFT, x, Δμ, ϵ) = increment!(Random.default_rng(), dist, x, Δμ, ϵ)

function increment!(rng::AbstractRNG, dist::MDFT, x, Δμ, ϵ)
    (;ν, S, σ, Δt, M) = dist
    n = length(ν)
    #W = fill(.5, 2)
    idx = rand(1:2)
