...
"""
mutable struct LCA{T<:Real} <: AbstrctLCA
    ν::Vector{T}
    α::T
    β::T
    λ::T
...
function params(d::AbstrctLCA)
    (d.ν, d.α, d.β, d.λ, d.τ, d.σ, d.Δt)    
end

get_pdf_type(d::AbstrctLCA) = Approximate

"""
    rand(dist::AbstrctLCA)
...
"""
function rand(rng::AbstractRNG, dist::AbstrctLCA)
    # number of trials 
    n = length(dist.ν)
...
"""
    rand(dist::AbstrctLCA, n_sim::Int)
...
"""
function rand(rng::AbstractRNG, dist::AbstrctLCA, n_sim::Int)
    n = length(dist.ν)
    x = fill(0.0, n)
    Δμ = fill(0.0, n)
    ϵ = fill(0.0, n)
...
"""
function simulate(model::AbstrctLCA; _...)
    (;Δt,α) = model 
    n = length(model.ν)
    x = fill(0.0, n)
    μΔ = fill(0.0, n)
