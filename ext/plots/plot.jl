...
plot!(d::SSM2D; t_range = default_range(d), kwargs...) =
    plot!(Plots.current(), d; t_range, kwargs...)
...
function default_range(d)
    n = n_options(d)
    return range(d.τ + eps(), d.τ + 0.25 * log(n + 1), length = 100)
end

function default_range(d::CDDM)
    (; ν, α) = d
    mag = 3 * log(1.0 + α / norm(ν))
    return [range(-π, π, length = 100), range(d.τ + eps(), d.τ + mag, length = 100)]
end
