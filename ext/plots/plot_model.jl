...
function get_default_labels(model)
    (; τ, α) = model
    return [(0, α, text("α", 10, :bottom)), (τ / 2, 0, text("τ", 10, :bottom))]
end
...
plot_bottom_density!(d::AbstractDDM, cur_plot; kwargs...) =
    plot_bottom_density!(get_pdf_type(d), d, cur_plot; kwargs...)
plot_top_density!(d::AbstractDDM, cur_plot; kwargs...) =
    plot_top_density!(get_pdf_type(d), d, cur_plot; kwargs...)
...
function plot_top_density!(
    ...
    return plot!(cur_plot, t_range, pds[1]; ylims = (ymin, ymax), defaults..., kwargs...)
end
...
function plot_top_density!(
    ...
    filter!(!isnan, pds_vec)
    defaults = get_plot_defaults(d)
    return plot!(cur_plot, t_range, pds[1]; defaults..., kwargs...)
end
...
function plot_bottom_density!(
    ...
    defaults = get_plot_defaults(d)
    dens = -1 * pds[2]
    return plot!(cur_plot, t_range, dens; defaults..., kwargs...)
end
...
function plot_bottom_density!(
    ...
    return plot!(cur_plot, t_range, dens; ylims = (ymin, ymax), defaults..., kwargs...)
end
